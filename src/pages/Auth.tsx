  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (error) throw error;
        toast({ title: "Account created! 🎉", description: "Welcome to EnuguExplore" });
        navigate('/');
      }
    } catch (error: any) {
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="border-0 shadow-xl">
          <CardHeader className="text-center pb-2">
            ...
            <CardDescription>{isLogin ? 'Sign in to continue exploring' : 'Join EnuguExplore today'}</CardDescription>
          </CardHeader>
